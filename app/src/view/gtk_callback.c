...
int g_callback_mkdir_popup_open(GtkWidget* widget, gpointer data){
    ...
    gtk_window_set_title(GTK_WINDOW(mkdir_modal.window), "MAKE DIRECTORY");
    // printf("menu open\n");
...
gboolean on_enter_notify(GtkWidget *widget, GdkEventCrossing *event, gpointer data) {
    file_info_t file = get_clicked_file_info(widget);
    if(file.name[0] == '\0'){
        return TRUE;
    }
...
int g_callback_file_rename(GtkWidget* widget, gpointer data){
    ...
    gtk_entry_set_text(GTK_ENTRY(mkdir_modal.inp_filename), file_list[selected_index].name);
    gtk_window_set_title(GTK_WINDOW(mkdir_modal.window), "RENAME FILE");
    gtk_widget_show_all(mkdir_modal.window);
}
...
int g_callback_file_mklink(GtkWidget* widget, gpointer data){
    ...
    gtk_window_set_title(GTK_WINDOW(mkdir_modal.window), "MAKE LINK FILE");
    gtk_widget_show_all(mkdir_modal.window);
}
